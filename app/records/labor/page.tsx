...
import { type Labor, columns } from "./columns"
...
export default function LaborRecordPage() {
  ...
  const fetchLaborRecords = useCallback(async () => {
    const { data: laborData, error } = await supabase.from("labor_adding").select("*").order("id", { ascending: false })
  ...
  return (
    <SidebarInset>
          ...
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem className="hidden md:block">
                <BreadcrumbLink href="/labor">Labor Management</BreadcrumbLink>
              </BreadcrumbItem>
      ...
      <div className="container mx-auto py-10">
        <DataTable columns={columns} data={data} loading={loading} refreshLabor={fetchLaborRecords} />
      </div>
    </SidebarInset>
  )
}
