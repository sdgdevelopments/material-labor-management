...
export default function MaterialRecordPage() {
  ...
  const fetchMaterials = useCallback(async () => {
    const { data, error } = await supabase.from("material_adding").select("*").order("id", { ascending: false })
    if (data) {
      setData(
        data.map((item) => ({
          id: item.id,
    ...
    <SidebarInset>
          ...
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem className="hidden md:block">
                <BreadcrumbLink href="/materials">Material Management</BreadcrumbLink>
              </BreadcrumbItem>
      ...
      <div className="container mx-auto py-10">
        {loading ? (
          ...
          <DataTable columns={columns} data={data} refreshMaterials={fetchMaterials} />
        )}
      </div>
    </SidebarInset>
  )
